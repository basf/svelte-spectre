...
describe('Index', () => {
    ...
    describe('once the component has been rendered', () => {
        test('should show the proper heading', () => {
            expect(
                renderedComponent.getByText('Welcome to SvelteSpectre')
            ).toBeInTheDocument();
        });

        test('Welcome the user', () => {
            const { getByText } = render(Index, { props: { name: 'John' } });

            expect(getByText('Welcome to SvelteJohn')).toBeInTheDocument();
        });
    });
});
